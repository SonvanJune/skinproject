...
@section('title', __('message.wishList'))
        ...
        @if (!empty($user_name))
            <div class="wishlist-container">
                <div class="wishlist-banner" style="background-image: url('{{ asset('images/breadcamb.jpg') }}')">
                    <div class="container">
                        <h2>{{ __('message.wishList') }}</h2>
                        <p class="breadcrumb">
                            <a href="{{ route('home') }}">{{ __('message.home') }}</a> <span>></span>
                            <span>{{ __('message.wishList') }}</span>
                        </p>
                        @isset($wistList)
                            @if (count($wistList->data) > 0)
                                <nav aria-label="Page navigation">
                                    <ul class="pagination justify-content-center">
                                        @if ($wistList->current_page != 1)
                                            <li class="page-item">
                                                <a class="page-link"
                                                    href="{{ url(App::getLocale() . '/wishlist' . '?page=' . $wistList->current_page - 1) }}"
                                                    aria-label="Previous">
                                                    <span aria-hidden="true">&laquo;</span>
                                                </a>
                                            </li>
                                        @endif
                                        @for ($i = 1; $i <= $wistList->last_page; $i++)
                                            @if ($wistList->current_page == $i)
                                                <li class="page-item active"><a class="page-link"
                                                        href="{{ url(App::getLocale() . '/wishlist' . '?page=' . $i) }}">{{ $i }}</a>
                                                </li>
                                            @else
                                            ...
                                            @endif
                                        @endfor
                                        @if ($wistList->current_page != $wistList->last_page)
                                            <li class="page-item">
                                                <a class="page-link"
                                                    href="{{ url(App::getLocale() . '/wishlist' . '?page=' . $wistList->current_page + 1) }}"
                                                    aria-label="Next">
                ...
                <div class="container">
                    <div class="grid-container mt-5">
                        @isset($wistList)
                            @if (count($wistList->data) > 0)
                                @foreach ($wistList->data as $product)
                                            ...
                                            <div class="item-action">
                                                ...
                                                <form action="{{ route('wishlist.add') }}" method="post">
                                                    ...
                                                    <input type="text" value="{{ $product->product_id }}" hidden
                                                        name="product_id">
                                                    <button class="see-more-btn icon-container" onclick="like()"><i
                                                            class="bi bi-trash"></i>
                                                        <span class="tooltip">{{ __('message.deleteButton') }}</span>
                                                    </button>
                                                </form>
                                            </div>
                                        </div>
