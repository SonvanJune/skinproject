...
@section('content')
            ...
            <div class="col-sm mb-2 mb-sm-0">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb breadcrumb-no-gutter">
                        ...
                        <li class="breadcrumb-item active" aria-current="page">Edit Slideshow</li>
                    </ol>
                </nav>

                <h1 class="page-header-title">Edit Slideshow</h1>
            </div>
            <!-- End Col -->
        </div>
        <!-- End Row -->
            ...
            @foreach ($slideshowImages as $slideshowImage)
                <div class="slide-item">
                    <div class="slide-image">
                        <img src="{{ route('get.file', ['filename' => $slideshowImage->slideshow_image_url]) }}"
                            alt="">
                    </div>
                    <div class="slide-dot"></div>
                    <div class="slide-index">{{ $slideshowImage->slideshow_image_index }}</div>
                </div>
            @endforeach
        </div>
    </div>

    <form class="row needs-validation justify-content-center" novalidate action="{{ route('admin.slideshows.update') }}"
        method="POST">
        @csrf
        @method('PUT')
        <input type="hidden" name="slideshow_image_id" value="{{ $slideImage->slideshow_image_id }}">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-4">
                        <input type="number" class="form-control" name="slideshow_image_index" id="statusLabel"
                            placeholder="eg. 10" value="{{ $slideImage->slideshow_image_index }}">
            ...
            <div class="card mb-3 mb-lg-5">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-4 dz-dropzone dz-dropzone-card dz-clickable">
                        ...
                        <div id="attachFile">
                            @if (!empty($slideImage->slideshow_image_url))
                                <div class="col-12 m-1 dz-preview dz-file-preview">
                                    <div class="dz-details d-flex flex-column align-items-center">
                                        <div class="dz-img flex-shrink-0">
                                            <img class="img-fluid" alt="{{ $slideImage->slideshow_image_alt }}"
                                                src="{{ route('get.file', ['filename' => $slideImage->slideshow_image_url]) }}">
                                        </div>
                                        <div class="dz-file-wrapper flex-grow-1 mt-2">
                                            <h6 class="dz-filename">
                                                <span class="dz-title" data-dz-name="">
                                                    {{ Str::afterLast($slideImage->slideshow_image_url, '/') }}</span>
                                            </h6>
                                            <div class="dz-size" data-dz-size="">
                                                <strong>{{ getFileSizeFormatted(base_path($slideImage->slideshow_image_url)) }}</strong>
                                            </div>
                                        </div>
                                    </div>
                                    <div>
                                        <input type="hidden" class="form-control" name="slideshow_image_url"
                                            value="{{ $slideImage->slideshow_image_url }}">
                                        <input type="text" class="form-control text-center" name="slideshow_image_alt"
                                            id="slideshow_image_alt" placeholder="Enter image alt..."
                                            value="{{ $slideImage->slideshow_image_alt }}" required>
    ...
    <script>
        ...
        const renderFile = (selectedFile) => {
            ...
            <div>
                <input type="hidden" class="form-control" name="slideshow_image_alt" value="${path}" required>
                <input type="text" class="form-control text-center" name="slideshow_image_url" placeholder="Enter image alt..." value="${selectedFile.filePath}" required>
                <div class="invalid-feedback"> Please enter image alt.</div>
            </div>`;
