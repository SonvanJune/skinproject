...
@section('title', __('message.newPopularTemplates'))
    ...
    <div class="wish-list">
        <div class="wishlist-container">
            <div class="wishlist-banner" style="background-image: url('{{ asset('images/breadcamb.jpg') }}')">
                <div class="container">
                    <h2>{{ __('message.newPopularTemplates') }}</h2>
                    <p class="breadcrumb">
                        <a href="{{ route('home') }}">{{ __('message.home') }}</a> <span>></span>
                        <span>{{ __('message.newPopularTemplates') }}</span>
                    </p>
                    @isset($populars)
                        @if (count($populars->data) > 0)
                            <nav aria-label="Page navigation">
                                <ul class="pagination justify-content-center">
                                    @if ($populars->current_page != 1)
                                        <li class="page-item">
                                            <a class="page-link"
                                                href="{{ url(App::getLocale() . '/popular-products' . '?page=' . $populars->current_page - 1) }}"
                                                aria-label="Previous">
                                                <span aria-hidden="true">&laquo;</span>
                                            </a>
                                        </li>
                                    @endif
                                    @for ($i = 1; $i <= $populars->last_page; $i++)
                                        @if ($populars->current_page == $i)
                                        ...
                                        @endif
                                    @endfor
                                    @if ($populars->current_page != $populars->last_page)
                                        <li class="page-item">
                                            <a class="page-link"
                                                href="{{ url(App::getLocale() . '/popular-products' . '?page=' . $populars->current_page + 1) }}"
                                                aria-label="Next">
            ...
            <div class="container">
                <div class="grid-container mt-5">
                    @isset($populars)
                        @if (count($populars->data) > 0)
                            @foreach ($populars->data as $product)
