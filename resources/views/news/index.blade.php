...
@section('title', __('message.newUploadTemplates'))
    ...
    <div class="wish-list">
        <div class="wishlist-container">
            <div class="wishlist-banner" style="background-image: url('{{ asset('images/breadcamb.jpg') }}')">
                <div class="container">
                    <h2>{{ __('message.newUploadTemplates') }}</h2>
                    <p class="breadcrumb">
                        <a href="{{ route('home') }}">{{ __('message.home') }}</a> <span>></span>
                        <span>{{ __('message.newUploadTemplates') }}</span>
                    </p>
                    @isset($news)
                            ...
                            <nav aria-label="Page navigation">
                                <ul class="pagination justify-content-center">
                                    @if ($news->current_page != 1)
                                        <li class="page-item">
                                            <a class="page-link"
                                                href="{{ url(App::getLocale() . '/new-products' . '?page=' . $news->current_page - 1) }}"
                                                aria-label="Previous">
                                    ...
                                    @for ($i = 1; $i <= $news->last_page; $i++)
                                        @if ($news->current_page == $i)
                                            <li class="page-item active"><a class="page-link"
                                                    href="{{ url(App::getLocale() . '/new-products' . '?page=' . $i) }}">{{ $i }}</a>
                                            </li>
                                        @else
