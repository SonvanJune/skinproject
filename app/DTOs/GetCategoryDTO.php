...
class GetCategoryDTO
{
    ...
    public static function fromModel(Category $category, array &$processedCategories = []): self|null
    {
        if($category){
            ...
            $childrenDTO = [];
            foreach ($category->childrens()->where('category_status', '!=' , Category::CATEGORY_STATUS_DELETE)->where('category_status', '!=' , Category::CATEGORY_STATUS_INACTIVE)->where('category_release', '<=', now())->get() as $_category) {
