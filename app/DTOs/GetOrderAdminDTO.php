...
class GetOrderAdminDTO
{
     ...
     */
    public static function fromModel(Order $order, UserService $userService): static
    {

        $cart = Cart::find($order->cart_id);
        $user = $userService->decryptUser($cart->user);
        return new static(
            $order->order_id,
            GetUserDTO::fromModel($user),
            GetCartDTO::fromModel($cart, $cart->user->user_id),
            $order->order_price,
            $order->order_payment,
     ...
     */
    public static function fromModels(Collection $orders, UserService $userService): array
    {
        $result = [];
        foreach ($orders as $order) {
            $user = $userService->decryptUser($order->cart->user);
